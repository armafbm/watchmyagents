...
export const Route = createFileRoute("/pricing")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Simple, transparent pricing for runtime AI agent security. Start free. Pay as you scale — from Starter to Enterprise." },
      { property: "og:title", content: "Pricing — WatchMyAgents" },
      { property: "og:description", content: "Start free. Pay as you scale. Plans from $0 to Enterprise." },
    ],
  }),
});
...
const TIERS: Tier[] = [
  ...
  { name: "Pro", agents: "3 to 10 agents", monthlyPrice: 29, monthlyAnnual: 20.3, priceIdMonthly: "pro_monthly", priceIdYearly: "pro_yearly", features: ["Advanced monitoring", "Smart alerts", "90-day history", "Email support (24h)", "Webhooks"], cta: "Subscribe" },
...
function PricingPage() {
  ...
  const handleSubscribe = (t: Tier) => {
    const priceId = billing === "monthly" ? t.priceIdMonthly : t.priceIdYearly;
    if (!priceId) return;
    if (!user) {
      navigate({ to: "/auth/signup", search: { redirect: "/pricing" } as never });
      return;
    }
    openCheckout({
      priceId,
      ...
      <main className="pt-24 pb-24 px-4 sm:px-6">
        <section className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h1 className="font-display text-4xl sm:text-6xl font-bold tracking-tight mt-4">Simple, transparent pricing</h1>
            <p className="text-muted-foreground mt-4 text-lg">Start free. Pay as you scale.</p>
          </div>

          <div className="flex justify-center items-center gap-3 mb-12">
            <span className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Billing:</span>
            <div className="inline-flex rounded-md border border-border overflow-hidden">
              {(["monthly", "annual"] as const).map((b) => (
                <button key={b} onClick={() => setBilling(b)} className={cn("px-4 py-2 font-mono text-xs uppercase tracking-widest transition-all", billing === b ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground")}>{b}</button>
              ))}
            </div>
            {billing === "annual" && (
              <span className="bg-primary/15 text-primary text-[10px] font-mono font-bold tracking-widest uppercase px-2.5 py-1 rounded">Save 30%</span>
            )}
          </div>

          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {TIERS.map((t) => {
              const isPaid = !!(t.priceIdMonthly || t.priceIdYearly);
              const btnClass = cn(
                "block w-full text-center font-mono text-xs uppercase tracking-widest px-4 py-3 rounded-md border transition-all",
                t.featured ? "bg-primary text-primary-foreground border-primary hover:opacity-90" : "border-primary/40 text-primary hover:bg-primary hover:text-primary-foreground"
              );
              return (
                <div key={t.name} className={cn("relative flex flex-col rounded-xl border p-7 backdrop-blur transition-all", t.featured ? "border-primary bg-primary/5 shadow-[0_0_40px_-10px_hsl(var(--primary)/0.4)]" : "border-border/60 bg-card/40 hover:border-border")}>
                  {t.badge && (
                    <span className="absolute -top-3 left-7 bg-primary text-primary-foreground text-[10px] font-mono font-bold tracking-widest uppercase px-3 py-1 rounded">{t.badge}</span>
                  )}
                  ...
                  <div className="flex items-baseline gap-1.5 mt-5">
                    ...
                    {t.monthlyPrice !== null && t.monthlyPrice !== 0 && (<span className="text-sm text-muted-foreground">/month</span>)}
                  </div>
                  ...
                  {isPaid ? (
                    <button onClick={() => handleSubscribe(t)} className={btnClass}>{t.cta}</button>
                  ) : t.ctaHref?.startsWith("mailto:") ? (
                    <a href={t.ctaHref} className={btnClass}>{t.cta}</a>
                  ) : (
                    <Link to={t.ctaHref!} className={btnClass}>{t.cta}</Link>
                  )}
                </div>
              );
            })}
        ...
        <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-md overflow-y-auto">
          <div className="max-w-3xl mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-6">
              <span className="font-mono text-xs uppercase tracking-widest text-muted-foreground">● Secure checkout</span>
              <button onClick={closeCheckout} className="h-9 w-9 grid place-items-center rounded-md border border-border/60 hover:border-primary/60 transition" aria-label="Close">
                <X className="h-4 w-4" />
              </button>
            </div>
            {checkoutElement}
