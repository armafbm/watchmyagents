...
async function buildContext(supabase: ReturnType<typeof createClient>, customerId: string) {
  const [agentsR, suggR, polR] = await Promise.all([
    ...
    supabase
      .from("suggestions")
      .select("title, risk_score, risk_category, objective, proposed_action, status, generated_at")
      .eq("customer_id", customerId)
      .order("generated_at", { ascending: false })
      .limit(15),
    supabase
      .from("policies")
      .select("rule_id, name, action, enabled, agent_id")
      .eq("customer_id", customerId)
      .limit(20),
  ]);
  return {
...
serve(async (req) => {
  ...
  const messages = [
    ...
    {
      role: "system",
      content: `CONTEXT (current tenant snapshot):\n${JSON.stringify(context, null, 2)}`,
    },
