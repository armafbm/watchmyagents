...
const incidents = [
  {
    sev: "CRITICAL",
    color: C.danger,
    agent: "agent.support · prod",
    cat: "Data access",
    signal: "Suspected exfiltration",
    t: 80,
  },
  {
    sev: "HIGH",
    color: C.warn,
    agent: "agent.crm · prod",
    cat: "Prompt injection",
    signal: "Malicious pattern in input",
    t: 110,
  },
  {
    sev: "WARN",
    color: C.primary,
    agent: "agent.finance · prod",
    cat: "Permissions",
    signal: "Scope=admin on read task",
    t: 140,
  },
  {
    sev: "INFO",
    color: "#5ce1a0",
    agent: "agent.ops · prod",
    cat: "Health",
    signal: "Heartbeat ok",
    t: 170,
  },
        ...
        <BrowserFrame url="fortress.watchmyagents.com/dashboard" width={1700} height={880}>
          <div style={{ display: "flex", height: "100%", background: C.bg }}>
            ...
            >
              ...
              {["Overview", "Watch", "Guardian", "Shield", "Legions", "Reports", "Settings"].map(
                (s, i) => (
                  <div
                    key={s}
                    style={{
            ...
            <div style={{ flex: 1, padding: 36, overflow: "hidden" }}>
              ...
              >
                <div>
                  <div
                    style={{ fontFamily: F.mono, fontSize: 11, letterSpacing: 4, color: C.muted }}
                  >
                    // OVERVIEW
                  </div>
                  <div style={{ fontFamily: F.display, fontSize: 38, color: C.fg, marginTop: 4 }}>
...
const Row: React.FC<{ sev: string; color: string; agent: string; cat: string; signal: string }> = ({
  sev,
  color,
  agent,
  cat,
