...
export const Scene4Landing: React.FC = () => {
  ...
  const scroll = interpolate(frame, [60, 140, 220, 300, 380], [0, 700, 1400, 2100, 2800], {
...
const FakeProblem: React.FC = () => (
  <div style={{ height: 700, padding: 80, background: C.bg }}>
    <span style={{ fontFamily: F.mono, fontSize: 12, letterSpacing: 4, color: C.danger }}>
      // THE PROBLEM
    </span>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
      {["Prompt injection", "Data exfiltration", "Permission drift"].map((t) => (
        ...
        >
          <div style={{ fontFamily: F.mono, fontSize: 12, color: C.danger, letterSpacing: 3 }}>
            RISK
          </div>
          <div style={{ fontFamily: F.display, fontSize: 32, color: C.fg, marginTop: 12 }}>{t}</div>
          <div style={{ fontFamily: F.body, fontSize: 16, color: C.muted, marginTop: 14 }}>
            Detected, scored and contained — automatically.
          </div>
        </div>
      ))}
    </div>
...
const FakeLoop: React.FC = () => (
  ...
  >
    <div style={{ textAlign: "center" }}>
      <span style={{ fontFamily: F.mono, fontSize: 12, letterSpacing: 4, color: C.primary }}>
        // THE LOOP
      </span>
      <h2 style={{ fontFamily: F.display, fontSize: 80, color: C.fg, margin: "16px 0 50px 0" }}>
        One closed feedback loop
      </h2>
        ...
        {["WATCH", "GUARDIAN", "SHIELD"].map((n, i) => (
          <div
            ...
            style={{
              ...
              height: 160,
              background: C.card,
              border: `1px solid ${[C.primary, C.accent, "#5ce1a0"][i]}66`,
              borderRadius: 14,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
...
const FakeLayers: React.FC = () => (
  <div style={{ height: 700, padding: 80, background: C.bg }}>
    <span style={{ fontFamily: F.mono, fontSize: 12, letterSpacing: 4, color: C.accent }}>
      // CAPABILITIES
    </span>
    <h2 style={{ fontFamily: F.display, fontSize: 70, color: C.fg, margin: "16px 0 40px 0" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
      {["Live observability", "Risk scoring", "Adaptive policies", "Native SIEM exports"].map(
        (t) => (
          <div
            key={t}
            style={{
...
const FakeCTA: React.FC = () => (
  ...
  >
    <div>
      <h2
        style={{ fontFamily: F.display, fontSize: 100, color: C.fg, margin: 0, letterSpacing: -3 }}
      >
...
const Cursor: React.FC = () => {
  const frame = useCurrentFrame();
  const x = interpolate(frame, [40, 200, 380], [200, 1200, 800], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const y = interpolate(frame, [40, 200, 380], [600, 200, 500], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  return (
